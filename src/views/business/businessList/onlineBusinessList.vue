    ...
    <div class="searchForm">
      ...
      <el-form :inline="true" :model="formInline" class="demo-form-inline" v-if='showFormBool'>
        <el-form-item label="输入搜索">
          <el-input v-model="formInline.keywords" placeholder="商家名称"></el-input>
        </el-form-item>
        <el-form-item label="商家类目">
          <el-select v-model='formInline.categoryIds' placeholder='请选择对应类目' clearable>
...
-->
        ...
        <el-table-column
          ...
          <template slot-scope='scope'>
            ...
            <el-button @click='goBusinessGoods(scope.row.businessId)'>商品管理</el-button>
            <el-button @click='detailPackages(scope.row.businessId)' style='margin-top: 4px;'>查看服务</el-button>
            <el-button @click='detailVouchers(scope.row.businessId)' style='margin-top: 4px;'>代金券</el-button>
          </template>
        </el-table-column>
      </el-table-column>
    </el-table>
...
<script>
  export default {
    ...
    mounted () {},
    methods: {
      detailPackages (businessId) {
        this.$router.push({path: '/business/unlineBusinessPackagesList', query: {businessId: businessId}})
      },
      detailVouchers (businessId) {
        this.$router.push({path: '/business/unlineBusinessVouchersList', query: {businessId: businessId}})
      },
      ...
      addBusiness (business_id) {
        if (business_id) {
          this.$router.push({path: '/business/onlineBusinessDetail', query: {business_id: business_id, type: 21}})
        } else {
          this.$router.push({path: '/business/onlineBusinessDetail'})
        }
      },
      goBusinessGoods (business_id) {
        this.$router.push({path: '/business/onlineBusinessGoodsList', query: {business_id: business_id}})
      }
    }
  }
</script>
